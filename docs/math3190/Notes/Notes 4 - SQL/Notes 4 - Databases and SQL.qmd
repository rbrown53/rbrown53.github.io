---
title: "Notes 4 - Databases and SQL"
...
In Quarto, you can insert an SQL code chunk. 
...
```
````
Be aware that the setup comments for SQL code chunks are `--|` instead of `#|` because an SQL comment is `--` whereas **R** and Python comments are both `#`. 

We will need to establish a connection before using the SQL code chunk, though. This connection must be to a database, either remote or local. 

## Define an SQL Connection in **R**

To connect to a remote database, we can use the following code chunk in Quarto. We will need a few SQL and database packages, though. Some common ones are `DBI`, `RMySQL`, and `RSQL`, and `RSQLite`. This database is a MySQL database, so we will need the `RMySQL` library. 
...
#| echo: false
#| eval: true
movies <- read.csv("../../Data/Sakila DB Data/film.csv")
pg_movies <- movies |>
  filter(rating == "PG") |>
  select(title, release_year, rating) |> tibble()
head(pg_movies, 5)
...
\footnotesize
```{r}
#| label: establish memory db
#| eval: true
#| echo: true
con <- dbConnect(RSQLite::SQLite(), ":memory:")
sakila_files <- list.files("../../Data/Sakila DB Data")
for (file in sakila_files) {
  data_file <- read.csv(paste0("../../Data/Sakila DB Data/", file))
...
or I can connect to the database in Quarto using
````{verbatim}
#| echo: true
```{sql}
--| connection: mammals_con
--| max-print: 5
SELECT * FROM surveys;
```
````
...
options(width = 60)
library(inborutils)
csv_to_sqlite(csv_file = "../../data_raw/combined.csv",
              sqlite_file = "../../data_raw/combined.sqlite", 
              table_name = "combined")
              
combined_con <- DBI::dbConnect(RSQLite::SQLite(), "../../data_raw/combined.sqlite")
dbGetQuery(combined_con, "SELECT * FROM combined") |> head()
```
