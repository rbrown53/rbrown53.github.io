...
include-in-header:
  text: |
    ...
    \setcounter{page}{88}
    \usepackage{fancyhdr}
    \fancyhead[l]{\small MATH 3190 -- NOTES 6}
    \pagestyle{fancy}
...
\begin{center} \fbox{\Large\textbf{Notes 6: Optimization}} \end{center}
...
1. Find $f'(x)$.
2. Set $f'(x)=0$ and solve for $x$ to find the critical number(s). 
3. Plug the critical number(s) and endpoint(s) into $f(x)$. If there are no endpoints, take a limit as $x$ approaches the end of the domain. 
...
Since any tangent line that does not have a slope of zero will intercept the $x$-axis somewhere, we will call the $x$ value where it does $x_{k+1}$. Then it will be true that $f'(x_k)=\dfrac{f(x_k)-0}{x_k-x_{k+1}}$. Then we can simply solving this for the $x_{k+1}$:
...
c(f(0.1699384), f(1.130901), f(-1.30084))
```
...
This first issue, converging to the wrong point, occurs if the function is not \textbf{convex}. A function is convex if the line segment between any two distinct points on the graph of the function lies above the graph between the two points. There are different types of convex functions.\\

\begin{itemize}[nosep]
\item $f$ is convex if and only if $f''(x)\ge 0$ for all $x$.
\item $f$ is called \textbf{strictly convex} if $f''(x)>0$ for all $x$. 
\item $f$ is called \textbf{strongly convex} if $f''(x)\ge m>0$ for all $x$. \\
\end{itemize}

A convex function is shaped like a cup $\cup$. A \textbf{concave} function is shaped like a cap $\cap$. If $f(x)$ is convex, then $-f(x)$ would be concave and vice versa. Many functions are neither convex nor concave.\

\end{minipage}
\begin{minipage}{.5\textwidth}
\hspace{.3in}\includegraphics[width = 2in]{optim_figs/convex_compressed.jpg}
\end{minipage}
...
\begin{align*}
y&=f(x_k- 0 f'(x_k))- f'(x_k)\cdot f'\Big(x_k- 0 f'(x_k)\Big)(\gamma-0)\\
&=f(x_k)-\gamma f'(x_k)^2.
\end{align*}
...
Notice that $f'(x_k)^2$ has been replaced with $\Vert \nabla f(\xb_k)\Vert^2$, which is the squared **Euclidean norm** of the gradient vector. That is, $\displaystyle\Vert \nabla f(\xb_k)\Vert^2=\sum_{i=1}^n\left(\dfrac{\partial f}{\partial x_i}\Big\vert_{\xb_k}\right)^2$. We can find this in **R** using either `norm(gradient, type = "2")^2` or `sum(gradient^2)`. 
...
Many times in probability, we study distributions that have known parameters. For example, $X_i\sim \text{Normal}(\mu=10,\sigma^2=25)$ or $X_i\sim\text{Exponential}(\theta=4)$ or $X_i\sim\text{Binomial}(n=10, p=0.4)$. But what if we don't know the parameters? \

\underline{Example}: Suppose we have a random sample of size 10 from a distribution that we know is exponential, so $f(x)=\dfrac{1}{\theta}e^{-x/\theta}$, but we are not sure what $\theta$ is. How can we estimate $\theta$? Here is the sample: 0.2886, 0.0017, 12.70, 12.02, 2.78, 3.23, 21.39, 7.55, 1.28, 2.44.

Note that $\theta$ is the mean and the SD. `mean(x)` = 6.36803 and `sd(x)` = 6.997012. Is one better? 

\vspace{1.5in}

One of the most common ways to estimate a parameter of a distribution is with maximum likelihood estimation. The idea behind maximum likelihood estimation (MLE) is to use knowledge of the pdf (or pmf) of a distribution to find a ``good" estimator for the parameter(s). We are trying to maximize something, so ideas from earlier in this set of notes will come in handy here. 

It is good to know the pdfs or pmfs of sone of the more common distributions:
\begin{center}
\renewcommand{\arraystretch}{2}
\begin{tabular}{|c|c|c|c|c|c|c|c|}
\hline
\textbf{Distribution Name} & \textbf{pdf or pmf} & \textbf{Support}\\
\hline
Normal & $f(x|\mu, \sigma)=\dfrac{1}{\sqrt{2\pi\sigma^2}}\exp\left\{-\frac{1}{2\sigma^2}(x-\mu)^2\right\}$ & $-\infty<x<\infty$\\
\hline
Exponential & $f(x|\theta)=\dfrac{1}{\theta}e^{-x/\theta}$ & $x>0$\\
\hline
Binomial & $\displaystyle  f(x|n, p)={n \choose x}p^x(1-p)^{n-x}$ & $x=0,1,\dots,n$\\
\hline
Poisson & $\displaystyle f(x|\lambda)=\dfrac{e^{-\lambda}x^\lambda}{x!}$ & $x=0,1,\dots$\\
\hline
\end{tabular}
...
\begin{enumerate}
\item Write down the pdf (or pmf) for the distribution your data came from. 
\item Find the likelihood function by multiplying the pdfs (or pmfs) together.
\item Take the natural log of the likelihood function.
\item Take the derivative with respect to the parameter you want to estimate.
\item Set the derivative equal to 0, solve for the parameter, and put a hat $(\text{ }\widehat{}\text{ })$ over it.
\item Repeat steps 4 and 5 if there are more parameters that need estimating. \\
\end{enumerate}

Note that the \textbf{\underline{likelihood function}} is identical to the joint pdf (or pmf) except we think of the likelihood as a function of the parameter rather than a function of the $x$ values.
...
<!-- % Example: find the max of f(x)=x/e^x in the traditional way and using the log.  -->
\vspace{2.5in}

\newpage

\underline{Example}: Find the maximum likelihood estimator for $\theta$ if we know our random sample came from a distribution with pdf $f(x)=\dfrac{1}{\theta}e^{-x/\theta}$. The sample is 0.2886, 0.0017, 12.70, 12.02, 2.78, 3.23, 21.39, 7.55, 1.28, 2.44.
\vspace{2.5in}


\underline{Example}: Find the MLEs for $\mu$ and $\sigma^2$ if $X_i\sim N(\mu,\sigma^2)$ for $i=1,\dots,n$. Are $\widehat{\mu}$ and $\widehat{\sigma}^2$ unbiased? 
...
\begin{figure}[h]
\centering
\includegraphics[width=1.7in]{optim_figs/CGfig.pdf}\quad
\includegraphics[width=1.7in]{optim_figs/PCGfig.pdf}
\caption{The image on the left tracks the convergence of the CG algorithm and the image on the right shows PCG. Both methods converged in two iterations.}
\label{fig:cg}
\end{figure}
