...
include-in-header:
  text: |
    ...
    \setcounter{page}{216}
    \usepackage{fancyhdr}
    \fancyhead[l]{\small MATH 3190 -- NOTES 11}
    \pagestyle{fancy}
...
\begin{center} \fbox{\Large\textbf{Notes 11: Time Series}} \end{center}
...
We can plot the autocorrelation using the `acf()` (included in base **R**) and `autoplot()` (in `ggplot2`) functions. Note that the `ggfortify` package must be loaded for this to work properly.
...
DJIA <- c(25426, 25543, 25439, 25883, 25891, 25954, 25851, 26032, 
         ...
         25473, 25450, 25651, 25555, 25703, 25710, 25849, 25914, 25978)
day <- 1:length(DJIA) 
autoplot(acf(DJIA, plot = F)) + theme_bw() + labs(title = "ACF For DJIA")
```
...
#| fig-height: 2.3
#| out.width: "60%"
autoplot(pacf(DJIA, plot = F)) + theme_bw() + labs(title = "PACF For DJIA")
```
...
We can also plot the time series with `ggplot2` formatting using the `autoplot()` function. For time series plots, like with the `acf()` and `pacf()` functions, the `ggfortify` package is also required. When plotting a time series object with `autoplot()`, we can add onto it as if it was a ggplot. Note that the `scale_x_yearmonth()` function is from the `tsibble` package and is useful when plotting time series.
...
Right now the `AirPassengers` data is of class `ts`, which means **R** knows it is a time series. However, it is not in tidy format. We can put it in a `tsibble`, which is just like a `tibble` but for time series.
...
Note that `tsbibble` objects work just like `ts` ones when it comes to the `autoplot()` function. But they can also be plotted with the `ggplot()` function like we are used to doing:
...
We can see that there is a cyclic nature to this time series (as is often the case) that repeats each year. This is known as the _seasonality_ component. Let's check out how air passenger numbers look by month. For many nice plotting functions, we can use the `ggtime` library. 
...
We can use the `gg_subseries()` (also in the `ggtime` package) to obtain another potentially useful plot. The average for each month is shown with the blue line.
...
Time series of this natures are called **stationary** if the mean and variance is constant across the series. There are several ways to model time series:
